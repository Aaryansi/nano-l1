...
plus a Shapley-value explainability layer that is validated against problems
where the correct attribution is known in advance. built on top of an existing
Go matching engine and dashboard.

**the headline result is negative, and that is the point.** the market is
efficient (calibration error 0.0172), the fee schedule imposes a 9% round-trip
hurdle, and nothing beats doing nothing. the agent's own shortfall is shown to
be its own measured bias rather than a fact about the market.

**the contribution is the explainability layer.** per-decision Shapley
attribution, the standard approach, is shown to be measurably misleading for a
sequential agent: on a corpus with a planted decoy it ranks a provably worthless
feature 3rd of 18, while return-based attribution ranks it 10th, and deletion
curves confirm the return-based ranking is the more faithful one. this
reproduces the argument of SVERL (Beechey, Smith and Şimşek, ICML 2023) from
scratch on a new domain; it does not advance the theory.

**how to reproduce.** `./reproduce.sh` regenerates every figure from public data
with no API keys. 214 tests. see **[docs/REPORT.md](docs/REPORT.md)** for the
results and **[docs/MDP.md](docs/MDP.md)** for the problem formulation.

```
services/agent-rl/     the rl agent, evaluation, and explainability  (phase 1-7)
...
> the sections below describe the original matching-engine project, which the
> RL work builds on and leaves intact.
