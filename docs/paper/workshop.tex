...
\twocolumn[
  ...
  one, and the problem is harder than it appears. The construction available off the
  shelf, randomising network parameters to generate reference draws, has almost
  no power: across sixteen checkpoints on four control tasks it never exceeds
  $z=2.96$, and its width closely tracks the standard deviation of
  random-initialisation return, so it measures the variance of initialisation
  and not the explanation. Run instead in its
  canonical form it answers a different question and clears the explanation we
  show to be empty. We then build the three natural replacements and
  ...
  respond, collapsing the reference onto a point mass, so that on two of our
  three cases it cannot decide at all. Permuting the outcomes, the direct analogue of the
  ...
  In each construction we built, removing the information also perturbed
  something else, and the perturbation decided the verdict. We motivate this with an agent whose explanation is
  stable across seeds, semantically legible, and empty, and which can be driven
  from $39.9\%$ to $3.2\%$ of attribution mass at no detected cost in return.
  \end{quote}
  \vspace{1.1em}
  \end{@twocolumnfalse}
]
...
sound, but it answers a different question and clears the explanation we show to
be empty.
...
return, which it tracks to within two percent on the tasks where either is
non-degenerate.
\item Three replacements, three distinct failures, each with a measured
mechanism.
\item A worked case of an explanation that passes every conventional check we
could apply and carries no information.
\item Attribution is not identified by task performance: it can be steered at a
cost that, in our controls, tracks whether the feature is load-bearing.
\end{enumerate}
...
construction, the demonstration that four references fail and why, and the
consequences.
...
The statistic needs a reference distribution but does not say how to build one,
which is this paper's subject. Every construction we try corrupts the
observation channel somewhere between corpus and agent
(Figure~\ref{fig:method}), differing in where and so in what else it disturbs.
Their failures give three requirements: preserve the observation stream, the
agent's responsiveness, and the difficulty of the task. Each construction below
sacrifices exactly one.

Given null draws with mean $\bar\Delta$ and standard deviation $s$ we report a
rank statistic and $z = (\Delta_{\mathrm{obs}} - \bar\Delta)/s$, and require both
to reject. We write the first as a rank rather than a calibrated $p$-value: an
exact randomisation $p$-value needs exchangeability under the null, which is
what this paper puts in doubt. The rank statistic assumes no shape but bottoms out at $1/(n+1)$; the
...
have cross-seed rank correlation $0.850$ with $100\%$ agreement on the top
...
informative, unchanged in all $10{,}000$ resamples. The trading agent on
identical episodes and an identical null gives $+7.41$ against $+2.47 \pm 2.29$,
$z=+2.16$, which the two-part rule declines, though only $66\%$ of resamples
...
very wide. Across sixteen checkpoints the parameter null never exceeds $z=2.96$,
including on a converged CartPole policy scoring 404 of 500, while the
environment null reaches $z=+153.3$. The two return the same verdict on eight of
the sixteen, and every one of those eight is a checkpoint where the observed
span is exactly zero and there is nothing to detect. On all five checkpoints
with a non-zero span they disagree, without exception.

\begin{table}[t]
\centering\small
\caption{Null construction on four control tasks, 24 draws each. MountainCar is degenerate under both: no policy in the budget reaches the goal. Pendulum's agent has a span of exactly zero at every checkpoint, so it constrains the widths but supplies no detection evidence.}
\label{tab:nulls}
\begin{tabular}{lrrr}
\toprule
Environment & Parameter & Environment & Ratio \\
\midrule
CartPole & $\pm \mathbf{140.86}$ & $\pm \mathbf{2.52}$ & $56\times$ \\
Acrobot & $\pm \mathbf{109.32}$ & $\pm \mathbf{0.00}$ & degenerate \\
MountainCar & $\pm \mathbf{0.00}$ & $\pm \mathbf{0.00}$ & $0\times$ \\
Pendulum & $\pm \mathbf{138.88}$ & $\pm \mathbf{27.50}$ & $5\times$ \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Why it is wide.} The gap has a mechanism, and it is not about
explanations. On every task the parameter null's width closely tracks the spread of
random-initialisation return: $140.43$ against $140.86$ on CartPole and
$110.59$ against $109.32$ on Acrobot, both inside two percent, and $127.40$
against $138.88$ on Pendulum, inside nine. In
...
initialisation}, which is why a test built on it has almost no power here.
Blinding draws each coordinate independently, which on Pendulum satisfies
$\cos^2 + \sin^2 = 1$ for only $0.5\%$ of draws. Resampling whole observation
rows instead, which stays on the manifold by construction, leaves all sixteen
verdicts unchanged and narrows the environment null from $2.52$ to $0.87$ on
CartPole, so the off-manifold draws were costing the test power rather than
manufacturing it.
...
rather than agent quality: its agent fails completely until it learns and the
span is exactly $0.00$ there. Its environment reference is a point mass at every
checkpoint, so the comparison is unresolved rather than negative, which is the
same failure blinding shows on our synthetic corpora. CartPole's observations
matter at every skill level and the test fires throughout, and it is the only
one of the four tasks supplying a non-zero span at every checkpoint. We do not claim
...
\begin{table}[t]
\centering\small
\caption{Steering one feature's attribution. Every figure in a row comes from one run at invariance penalty $\lambda=20$, fixed in advance. $p$ is a cluster bootstrap against the $\lambda=0$ agent, resampling seeds and then episodes within seeds.}
\label{tab:steer}
\begin{tabular}{lrrr}
\toprule
...
suppressing the planted signal destroys $81\%$ of return, while on the market no
return difference is detected. A non-significant comparison is not evidence of
equivalence, so we bound the effect instead of asserting one. The market moves
by $+1.84$ per episode, $95\%$ interval $[-1.18, +5.46]$; the planted signal by
$-36.38$, interval $[-46.04, -18.98]$. \emph{The intervals do not overlap}: the
largest cost the market experiment cannot exclude is a sixteenth of the smallest
cost consistent with the planted signal. Explanations are steerable without a
detected cost exactly where they are not tracking anything, corroborating
Section 4.1 by a route sharing no machinery with the null test. The control matters: had steering succeeded on both, the
...
point is that task performance does not pin it down. On $4{,}096$ held-out
states the steered and unsteered agents agree on the greedy action only
$66.0\%$ of the time, so they are not the same policy and the claim is about
performance rather than behaviour.
...
proposed for the $56\times$ gap (that randomising weights perturbs the
