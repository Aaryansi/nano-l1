...
% no changes.
%
% Length: about thirteen pages of body, with the bibliography running to
% fifteen. That suits arXiv or a journal. The paper grew when the null-
% construction result went from one construction to four, and the structure
% follows that result rather than the order it was discovered in.
%
% For a conference limit of eight pages excluding references, cut in this order,
% which is by how much each section would cost in review if it were missing:
%
%   1. "How much edge is required" and "Estimation guarantees do not
%      substitute". Both are single points that survive as one sentence each in
%      the limitations.
%   2. the integrated-gradients comparison inside "What the verdict does and
%      does not depend on", keeping its conclusion.
%   3. "Off-manifold masking", keeping the two-sentence version of its result:
%      the span is immune by construction, the ranking is not.
%   4. "Confidence intervals on the verdicts", folding the two unstable numbers
%      into the sections that report them.
%
...
construction the field already uses, randomising network parameters, has almost
no power: across sixteen checkpoints on four control tasks it never exceeds
$z=2.45$, and its width \emph{equals} the standard deviation of
random-initialisation return to three significant figures, so it measures the
variance of initialisation rather than anything about the explanation. We then
...
The pattern is that removing the information always perturbs something else, and
the perturbation decides the verdict. We motivate this with a worked case in
...
values~\citep{shapley1953value,lundberg2017unified}, integrated
...
negative result is the contribution. The construction reinforcement learning
already uses has almost no power, for a reason that turns out to be an identity
rather than a tendency. Each of the three natural ways to build a better one
fails, and each fails differently: removing the information also perturbs
something else, and it is the perturbation, not the information, that decides
the verdict. \emph{Choosing the reference distribution is the hard part of
...
\item \textbf{The incumbent has no power, and we give the mechanism.} Parameter
randomisation never exceeds $z=2.45$ across sixteen checkpoints on four control
tasks, including on a converged CartPole policy scoring 404 of 500, where an
environment-level null reaches $z=117$. Its width \emph{equals} the standard
deviation of random-initialisation return to three significant figures, so it
measures the variance of initialisation and not the explanation
...
seeds, separated beyond their own estimation error, and legible. Every
conventional check passes (Section~\ref{sec:empty}), and a positive control on
the same real episodes shows the test separates tasks rather than datasets
...
the observation, and report what that is worth. Six predictions this project
made were rejected on its own evidence and are collected rather than dropped
...
command; 295 tests cover the environment, the attribution implementations, and
...
again, and the mechanism is the one above rather than anything about horizon.
...
may well work. We have no evidence either way, and say so rather than
...
not have one it can rely on. The construction the field uses measures the
variance of initialisation. Of the three natural replacements, one varies the
...
can pass every conventional check and be empty, and it can be driven to say
