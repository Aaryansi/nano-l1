...
The results in this paper use the signal-free-corpus null. Its weakness, that it
also varies the corpus, is real and is measured in Section~\ref{sec:nullchoice};
it is the lesser of the two faults, because it widens the reference and so errs
toward declining rather than toward firing. We recommend against the blinded
form for tasks where inaction is a viable policy, which includes every
market-like task we know of, and we would not have learned that by reasoning
about which null was more principled.
...
Four predictions in this project turned out to be wrong
