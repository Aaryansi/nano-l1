...
answers a different question and points the opposite way: it clears the empty
explanation and flags the informative one. We then
...
choice almost no paper reports making.
  \end{quote}
  \vspace{1.2em}
  \end{@twocolumnfalse}
]
...
answers a different question, and on our two agents it points the other way. Each of the three natural ways to build a better reference fails, and each
...
explaining a reinforcement learning agent, and it is a choice almost no paper
reports making.}
...
clearing our empty explanation at $\rho=0.38$ and failing our informative one at
$\rho=1.00$ (Section~\ref{sec:canonical}).
...
same real episodes shows the test separates tasks and not datasets
...
The cost of forging an explanation is the importance of the feature, which is
zero exactly where a guarantee would matter (Section~\ref{sec:steering}).
...
test lacks an RL analogue. We supply a candidate, report that it fails, and in
Section~\ref{sec:canonical} run the canonical parameter test alongside it to
show the two answer different questions.
...
that learned nothing. Construction (a) randomises the policy's
...
The two verdicts run opposite to the ones our test gives. The market agent,
whose outcome-attribution span is indistinguishable from agents trained where
there is nothing to learn, \emph{passes}: fully randomised, its explanation
retains only $\rho=0.38$ of the original ranking, so it does depend on the
learned weights. The planted-signal agent, whose span fires at $z=12.27$,
\emph{fails}: at $\rho=1.00$ its explanation is the same whether the network is
trained or random. The two tests are applied to different explanatory targets,
behaviour for the canonical check and outcomes for ours, so this is not one
statistic contradicting itself. It is the sharper problem that a practitioner
running the canonical check on the market agent would find nothing wrong and
would be no closer to knowing whether there was anything to explain.

Both results follow from one mechanism. On the planted corpus a single feature
carries the signal by construction and dominates the input scale, so perturbing it moves the policy
output most whatever the weights are. The attribution ranking is recoverable
from the input distribution alone, which is exactly the failure mode the
canonical check is built to expose, and it exposes it. Our own null test is
blind to that failure, because a span can be large for input-driven reasons.

So the two tests answer different questions, and neither subsumes the other. The canonical check asks whether an explanation depends on
the learned parameters. The null test asks whether there is outcome-relevant
information to explain at all. An explanation can depend on the weights and
describe nothing, which is the market agent, or be largely input-determined
while sitting on a real signal, which is the planted agent. A practitioner who
runs only the canonical check would clear the empty explanation and flag the
informative one.

Three caveats. The agents live on different corpora, so agent and corpus are not
...
trained weights at all. The two tests target different explanations, as above,
so the comparison is between what a practitioner would conclude from each and
not between two estimates of one quantity. And two agents are an existence proof
that the conclusions can point opposite ways, not a rate at which they do.
...
is the one we would trust. Where the decision is consequential enough that none
...
\textbf{Rejected.} They point opposite ways. The market agent's explanation
depends on its weights ($\rho=0.38$) while carrying no detectable
outcome-relevant information; the planted agent's is nearly unchanged by full
randomisation ($\rho=1.00$) while sitting on a real signal
...
\textbf{The canonical check is run on one explanatory target.} Section~\ref{sec:canonical}
degrades behaviour-level attributions, because outcome-level attribution costs a
full batch replay per coalition and the check needs thirty recomputations per
agent. Whether the same opposition holds when both tests read the same target is
open, and it is the experiment we would run next with more compute.
...
intended it stays sound, but it answers a different question and on our two
agents points the other way. Of the three natural replacements, one varies the
